...
@section('content')
        ...
        <div class="p-5 text-center m-auto w-3/4 flex">
            <a href="{{ asset('/') }}" >
        ...
        <div class="flex justify-center">
            <div class="xl:w-3/4 p-5 sm:w-11/12">
                @forelse ($rooms as $room)
                    <h2 class=" text-3xl font-light text-left text-gray-100 py-3">{{$room->name}}</h2><br>

                    <div class="bg-gray-100 rounded-lg overflow-auto m-3 bg-opacity-90">
                       <div class="flex flex-wrap gap-4 justify-center m-5">
                           <a href="/device/{{ asset('create') }}">
                               <div class="my-1 bg-green-500 text-white uppercase rounded-full py-3 px-6 transition hover:bg-green-400 hover:text-white w-60 text-center">
                                   Add device
                               </div>
                           </a>
                           <a href="/settings/{{ $room -> id }}/edit">
                               <div class="my-1 bg-yellow-500 text-white uppercase rounded-full py-3 px-6 transition hover:bg-yellow-400 hover:text-white w-60 text-center">
                                   Modify room
                               </div>
                           </a>
                           <form action="/delete" method="POST">
                               @csrf<button class="uppercase my-1 uppercase bg-red-500 text-white rounded-full py-3 px-6 transition hover:bg-red-400 hover:text-white w-60 text-center" name="id" type="submit" value="{{ $room -> id }}">Delete room</button>
                           </form>
                       </div>
                                ...
                                <table class="m-5 p-2 bg-gray-100 hover:bg-gray-200 bg-opacity-75 hover:bg-opacity-95 rounded-lg">
                                    ...
                                    <tbody>
                                            <tr>
                                                <td class="p-3 m-1/3">
                                                    <ul class="m-1">
                                                        <li><span class="font-bold">Name:</span> {{ $esp->name }} </li>
                                                        <br>
                                                        <li><span class="font-bold">Type:</span> {{ $esp->type }} </li>
                                                        <br>
                                                        <li><span class="font-bold">IP:</span> 192.168.200.<span class="font-semibold">{{ $esp->ip_End }}</span></li>
                                                    </ul>
                                                </td>
