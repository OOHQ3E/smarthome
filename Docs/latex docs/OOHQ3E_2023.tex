...
\begin{document}
	...
	Ebben az esetben azért lett így összekapcsolva az ESP32-CAM egy FTDI Programmer-el,\footnote{Magyarul: FTDI programozó} hogy a vezérlőkódot fel tudjam rá tölteni. Az adatátvitelt az U0R - TX és az U0T - RX portok összekötése biztosítja. A GPIO 0 - GND kód feltöltéséhez kell, abban az esetben, amikor töltenénk fel a kódot a kamerára, akkor ahhoz meg kell nyomni a gombot, mint ahogyan \aref{esp32-cam-schematics}.~képen is látható. Az 5V - VCC a tápellátás, a GND - GND a földelés. Ha a programozását elvégeztük, akkor elegendő a tápot biztosítani az ESP32-CAM-nek.
	...
	\end{itemize}
	Ezeken kívül más metódusokat is szabadon létrehozhatunk. Én például ezt tettem az eszköz kapcsoláshoz, az \emph{,,EspController''}-en belül a \emph{,,Toggle(\$esp,\$status)''} metódussal, amit majd \aref{csatlakozas-a-webszerverre}.~szakaszon kifejtek.
	\\
	\Aref{roomcontroller-edit}.~programkódban pedig egy példa látható, hogy hogyan is adhatunk vissza egy adott rekordot egy adott nézettel.\\
	\begin{minipage}{1\linewidth}
		\centering
		\label{return-room}\includegraphics[width=1\textwidth]{./src/codes/return_view}
		\captionof{lstlisting}{\emph{,,RoomController''} edit metódusa}
		\label{roomcontroller-edit}
	\end{minipage}
	\\
	\Aref{roomcontroller-edit}.~programkódban az látható, hogy azt a nézetet adja vissza, ami egy szoba módosítására való, egy adott szoba rekorddal. Amikor visszaadunk egy nézetet, akkor lehetőség van arra, hogy ne csak egy adott rekordot adjunk vissza, hanem több rekordot, vagy rekord listákat is. Arra pedig itt egy példa kód.\\
	\begin{minipage}{1\linewidth}
		\centering
		\includegraphics[width=1\textwidth]{./src/codes/return_lists}
		\captionof{lstlisting}{\emph{,,RoomController''} index metódusa}
	\end{minipage}
	\\
	...
	Például \aref{roomcontroller-edit}.~programkódban, ahol egy szobát adunk vissza a módosítás nézetbe, annak szobának a nevét így érhetjük el: \emph{\textbf{,,\{\{\$room->name\}\}''}}
	...
	\begin{figure}[ht!]
		...
		\label{rfid-settings}
	\end{figure}
	Ezen az oldalon a beállítások oldalhoz majdnem ugyanúgy van kilistázva, hogy milyen RFID olvasó és hozzá tartozó RFID címkék, és azok adatai, mint ahogyan \aref{rfid-settings}.~képen látható.\\
	...
	Amikor hozzáadásra kerül a rendszerbe egy hőmérséklet és páratartalom szenzor, akkor az az által olvasott adatokat meg tudjuk nézni egy grafikonon. Amikor egy szenzoros panelre kattintunk a főoldalon. Ha rákattintunk, akkor \aref{temp-hum-chart}.~képen látható oldalhoz hasonlót fogunk látni.
	...
	Mivel az alkalmazott eszközök \emph{ESP-WROOM-32}-esekkel van megoldva, ezért valahogyan ki kellett gondolni, hogy ezeket hogyan is alkalmazzam, és hogyan is működjenek. Itt ezen a részen ezt részletezem. Az eszközöknek az adat folyamata \aref{esp-laravel.communication}.~ábrán szemléltetem, amit plantUML-lel készítettem el.
	...
	\end{itemize}
	\Aref{cypress-test}.~képen pedig az látható, hogy az összes teszt sikeresen lefutott.
	...
	Olyan tesztelések végeztem manuálisan, amelyeket nem lehetséges automatizált teszteléssel elvégezni: \Aref{rfid-toggle-test-table}.~táblázatban az RFID olvasó és a kapcsoló eszköz teszteléseit, \aref{camera-test-table}.~táblázatban pedig a kamera teszteléseit jegyeztem fel.
	\begin{table}
